...
def validate_subnets(subnets):
    ...
    for name, cidr in subnets.items():
        ...
        try:
            ...
            return f"Некорректный CIDR для сегмента '{name}': {cidr}"

    for i in range(len(net_objects)):
        for j in range(i + 1, len(net_objects)):
            if net_objects[i][1].overlaps(net_objects[j][1]):
                return f"Пересечение подсетей: '{net_objects[i][0]}' и '{net_objects[j][0]}'"
    return None
...
def validate_rules(rules, all_segments):
    ...
    for src, dst, svc in rules:
        if src not in all_segments or dst not in all_segments:
            errors.append(f"Неизвестный сегмент в правиле: {src} → {dst}")
            continue
        key = (src, dst, svc)
        if key in seen:
            errors.append(f"Дублирующее правило: {src} → {dst} по {svc}")
        seen.add(key)
